...
contract GreenPointLandReserves {
    ...
    uint256 public GLR_funds;
    uint256 public devPot;
    ...
    function globalData()
        ...
        view
        returns (
            uint256 _MVT_to_rollout,
            uint256 _mvt5xHodlPool,
            uint256 _nextFloorRaisingTime,
            uint256 _floorPrice,
            uint256 _totalACRESupply,
            uint256 _totalAcreWeight,
            uint256 _totalTotemWeight
        )
    {
        return (
            MVT_to_rollout,
            mvt5xHodlPool,
            nextFloorRaisingTime,
            floorPrice,
            _totalSupply,
            totalShares[ETHpool],
            totalTotemWeight
    ...
    function userData(address account)
        ...
        returns (
            uint256 acreBalance,
            uint256 totemWeight,
            uint256 acreDividends,
            uint256 totemDividends,
            bool MDT_approval,
            bool MVT_approval
        )
    {
        return (
            balanceOf(account),
            shares[MVTpool][account],
            dividendsOf(ETHpool, account) + earnings[ETHpool][account],
            dividendsOf(MVTpool, account) + earnings[MVTpool][account],
            MDT.allowance(account, THIS) > $ * 1000000,
            MVT.allowance(account, THIS) > $ * 1000000
        );
    }

    function userData2(address account)
        public
        view
        returns (
            uint256 MDT_balance,
            uint256 MVT_balance,
            uint256 colorDividends
        )
    {
        return (
            MDT.balanceOf(account),
            MVT.balanceOf(account),
            colorDividendsOf(account) + earnings[COLORpool][account]
        );
    }

    uint256 mvt5xHodlPool;
    event PurchaseAcre(address boughtFor, uint256 acreBought);
    ...
    function purchaseAcre(address buyFor) public payable {
        ...
        uint256 forDev;
        if (block.timestamp - genesis <= 86400 * 365) {
            forDev = (MONEY * 6) / 1000;
        }
        devPot += forDev;

        uint256 val = MONEY - forDev;
        mint(buyFor, val);
        uint256 forBuyingMVT = (val *
            (_totalSupply - totalTotemWeight + builder_totalShares)) /
            _totalSupply;
    ...
    uint256 nextFloorRaisingTime;
    uint256 floorPrice = 0.00002 ether;
    bool firstBump = true;
    event Sell_MVT(
        uint256 mvtSold,
        uint256 cashout,
    ...
    function sell_MVT(uint256 amount) public {
        ...
        uint256 cost = (floorPrice * amount) / $;
        require(mvt5xHodlPool >= cost && cost > 0);
        mvt5xHodlPool -= cost;

        uint256 forManifest = (amount *
            (totalTotemWeight - builder_totalShares)) / _totalSupply;
        uint256 forDaily = amount - forManifest;
    ...
    mapping(uint256 => uint256) earningsPer;
    mapping(uint256 => mapping(address => uint256)) payouts;
    mapping(uint256 => mapping(address => uint256)) public earnings;
    uint256 constant scaleFactor = 0x10000000000000000;
    uint256 constant ETHpool = 0;
    uint256 constant MVTpool = 1;
    uint256 constant COLORpool = 2;

    function withdraw(uint256 pool) public {
        address payable sender = payable(msg.sender);
        ...
        uint256 earned = earnings[pool][sender];
    ...
    function dividendsOf(uint256 pool, address account)
        public
        view
        returns (uint256)
    {
        uint256 owedPerShare = earningsPer[pool] - payouts[pool][account];
        return (shares[pool][account] * owedPerShare) / scaleFactor;
    }

    function colorDividendsOf(address account) public view returns (uint256) {
        uint256 owedPerShare = earningsPer[COLORpool] -
            payouts[COLORpool][account];
        return (shares[ETHpool][account] * owedPerShare) / scaleFactor;
    }

    function update(uint256 pool, address account) internal {
    ...
    event PayEthToAcreStakers(uint256 amount);

    function payEthToAcreStakers() public payable {
    ...
    mapping(uint256 => uint256) public builder_shares;
    uint256 public builder_totalShares;
    uint256 builder_earningsPer;
    mapping(uint256 => uint256) builder_payouts;
    mapping(uint256 => uint256) public builder_earnings;

    function builder_addShares(uint256 TOTEM, uint256 amount) internal {
    ...
    function builder_removeShares(uint256 TOTEM, uint256 amount) internal {
        if (!totemManifest[TOTEM]) {
            builder_update(TOTEM);
            builder_totalShares -= amount;
            builder_shares[TOTEM] -= amount;
    ...
    function builder_dividendsOf(uint256 TOTEM) public view returns (uint256) {
        uint256 owedPerShare = builder_earningsPer - builder_payouts[TOTEM];
        return (builder_shares[TOTEM] * owedPerShare) / scaleFactor;
    }

    function builder_update(uint256 TOTEM) internal {
        uint256 newMoney = builder_dividendsOf(TOTEM);
        builder_payouts[TOTEM] = builder_earningsPer;
        builder_earnings[TOTEM] += newMoney;
    }

    uint256 public MVT_to_rollout;
    uint256 public lastRollout;

    event DepositMVTForRewards(address addr, uint256 amount);

    function depositMVTForRewards(uint256 amount) public {
        require(MVT.transferFrom(msg.sender, THIS, amount));
        storeUpCommunityRewards(amount);
        emit DepositMVTForRewards(msg.sender, amount);
    }

    function storeUpCommunityRewards(uint256 amount) internal {
        if (builder_totalShares == 0) {
            storedUpBuilderMVT += amount;
        } else {
            builder_earningsPer +=
                ((amount + storedUpBuilderMVT) * scaleFactor) /
                builder_totalShares;
            storedUpBuilderMVT = 0;
        }
    }

    event RolloutDepositedMVTRewards(uint256 amountToDistribute);

    function rolloutDepositedMVTRewards() public {
        uint256 NOW = block.timestamp;
        if (
            (NOW - lastRollout) > 86400 &&
            totalShares[MVTpool] > 0 &&
            MVT_to_rollout > 0
        ) {
            lastRollout = NOW;
            uint256 amountToDistribute = (MVT_to_rollout *
                (totalTotemWeight - totalShares[MVTpool])) / _totalSupply;
            MVT_to_rollout -= amountToDistribute;
            earningsPer[MVTpool] +=
                (amountToDistribute * scaleFactor) /
                totalShares[MVTpool];
    ...
    function clean(address addr) internal {
        uint256 _shares = shares[ETHpool][addr];
        if (_shares > 0) {
            removeShares(ETHpool, addr, _shares);
            uint256 earned = earnings[ETHpool][addr];
    ...
    mapping(address => uint256[]) public totemsHad;
    mapping(address => mapping(uint256 => bool)) public alreadyHadAtleastOnce;

    uint256 totalTotemWeight;
    event AcreToTotem(address account, uint256 amount, bool autoStake);

    function acreToTotem(uint256 amount, bool autoStake)
        public
        returns (uint256 TOTEM_ID)
    {
        address sender = msg.sender;
    ...
    uint256 storedUpBuilderMVT;
    event TotemToMDT(address lastOwner, uint256 totemID, bool preventBurn);
    mapping(uint256 => bool) public totemManifest;

    function totemToMDT(uint256 totemID, bool preventBurn) public {
        address sender = msg.sender;
        require(
            sender == staker[totemID] &&
                !totemManifest[totemID] &&
                !requestLocked[totemID]
        );
        require(MDT.transfer(sender, $));
        uint256 totemWeight = totemNFT.getWeight(totemID);
        ...
        storeUpCommunityRewards(
            builder_dividendsOf(totemID) + builder_earnings[totemID]
        );
    ...
    event StakeNFT(address who, uint256 tokenID);
    ...
    event UnstakeNFT(address unstaker, uint256 tokenID);

    function unstakeNFT(uint256 tokenID) public {
        address sender = msg.sender;
        require(
            staker[tokenID] == sender &&
                !requestLocked[tokenID] &&
                block.timestamp - lastMove[tokenID] >= 86400
        );
        uint256 weight = totemNFT.getWeight(tokenID);
    ...
    function viewTotems(address account, uint256[] memory totems)
        public
        view
        returns (
            uint256[] memory tokenIDs,
            bool[] memory accountIsCurrentlyStaking,
            uint256[] memory acreWeight,
            bool[] memory owned,
            bool[] memory manifested,
            bool[] memory staked,
            uint256[] memory manifestEarnings,
            uint256[] memory lastMoved,
            bool[] memory pendingManifest
        )
    {
        uint256 L;
        if (totems.length == 0) {
        ...
        for (uint256 c = 0; c < L; c += 1) {
            ...
            manifestEarnings[c] =
                builder_dividendsOf(tID) +
                builder_earnings[tID];
            lastMoved[c] = lastMove[tID];
            owned[c] = (staker[tID] == account ||
                totemNFT.ownerOf(tID) == account);
        }
    }
    ...
    mapping(uint256 => address) public theWork; //noita
    mapping(uint256 => uint256) workingTotem;
    mapping(uint256 => string) public txt;
    mapping(uint256 => bool) requestLocked;
    event OracleRequest(
        address buidlr,
        uint256 totemID,
        uint256 earningsToManifest,
        address _theWork,
        string text,
        uint256 ticketID
    );
    ...
    function oracleRequest(
        uint256 totemID,
        string memory _txt,
        address contract_optional
    ) public payable returns (uint256 ticketID) {
        address sender = msg.sender;
        require(
            staker[totemID] == sender &&
                !totemManifest[totemID] &&
                !requestLocked[totemID]
        );
        uint256 ID = ORACLE.fileRequestTicket{value: msg.value}(1, true);
    ...
    event CommunityReward(
        ...
        uint256 totemID,
        uint256 reward,
        address contractBuilt,
        string text,
        uint256 ticketID
    );
    event RequestRejected(uint256 totemID, uint256 ticketID);
    ...
    function oracleIntFallback(
        ...
        uint256 positive;
        uint256 negative;
        uint256 totemID = workingTotem[ticketID];
        require(msg.sender == address(ORACLE));

        for (uint256 i; i < numberOfOptions; i += 1) {
        ...
        if (!requestRejected && positive > negative) {
            ...
            if (!totemManifest[totemID]) {
                ...
                uint256 earned = builder_earnings[totemID];
                if (earned > 0) {
...
contract Totem is ERC721 {
    ...
    mapping(uint256 => string) desiredURI;
    mapping(uint256 => uint256) workingTotem;
    event URI_request(uint256 totemID, string desiredURI, uint256 ticketID);

    function uriRequest(uint256 ID, string memory _desiredURI)
        public
        payable
        returns (uint256)
    {
        require(msg.sender == ownerOf(ID));
        uint256 otID = GreenPointLandReserves(greenpoint)
            .ORACLE()
            .fileRequestTicket{value: msg.value}(1, true);
    ...
    event AcceptedURI(uint256 totemID);
    event RejectedURI(uint256 totemID);
    ...
    function oracleIntFallback(
        ...
        require(
            msg.sender == address(GreenPointLandReserves(greenpoint).ORACLE())
        );
        ...
        if (
            !requestRejected &&
            positive > negative &&
            !GreenPointLandReserves(greenpoint).totemManifest(totemID)
        ) {
